...
function PlantsWithRecentImagesCard({ user }) {
  const [count, setCount] = useState(0);
  const [loading, setLoading] = useState(true);
  const { getPlantsWithRecentImages } = useApi();
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        setLoading(true);
        const plants = await getPlantsWithRecentImages();
        setCount(plants.length);
      } catch (error) {
        console.error('Failed to fetch plants with recent images:', error);
        setCount(0);
      } finally {
        setLoading(false);
      }
  ...
  const handleClick = () => {
    navigate('/plants?hasRecentImages=true');
  };
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 flex items-center justify-between">
          <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
            {count} plant{count !== 1 ? 's' : ''} {count > 0 ? 'with recent images' : 'no recent images'}
          </p>
...
export default PlantsWithRecentImagesCard;
